...
func main() {
	...
	// replace mentions to the representative mention.
	rep := ""
	for _, m := range r.Document.Coreference.Coreference.Mention {
		idx, _ := strconv.Atoi(m.Sentence[0].Text)
		s, _ := strconv.Atoi(m.Start.Text)
		e, _ := strconv.Atoi(m.End.Text)

		if m.AttrRepresentative == "true" {
			rep = ""
			for i := s; i < e; i++ {
				if i != s {
					rep += " "
				}
				rep += r.Document.Sentences.Sentence[idx-1].Tokens.Token[i-1].Word.Text
			}
		} else {
			r.Document.Sentences.Sentence[idx-1].Tokens.Token[s-1].Word.Text =
				"[" + rep + "] (" + r.Document.Sentences.Sentence[idx-1].Tokens.Token[s-1].Word.Text
			r.Document.Sentences.Sentence[idx-1].Tokens.Token[e-2].Word.Text += ")"
		}
	}
